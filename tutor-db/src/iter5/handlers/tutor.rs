...
use crate::iter5::{delete_tutor_db, get_all_tutors_db, get_tutor_details_db, post_new_tutor_db, update_tutor_details_db, AppState, EzytutorError, NewTutor, UpdateTutor};
...
pub async fn delete_tutor(
    ...
    delete_tutor_db(&app_state.db, tutor_id).await.map(|tutor| HttpResponse::Ok().json(tutor))
}